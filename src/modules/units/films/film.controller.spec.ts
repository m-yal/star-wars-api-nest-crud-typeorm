...
const mockFilms: Film[] = mocksPair.getMockUnits();
...
describe('Films controller tests:', () => {
    ...
    it("create method accepts input dto", async () => {
        ...
        const expectedResult: Film = plainToInstance(Film, { ...dto });

        const result: Film = await controller.create(dto);
    ...
    it("update method accepts input dto", async () => {
        ...
        const result: Film = await controller.update(dto);
...
class MockFilmService {
    create = jest.fn().mockImplementation(async (film: Film) => {
    ...
    update = jest.fn().mockImplementation(async (film: Film) => film);
    ...
    getUpToTen = jest.fn().mockImplementation(async (page: number): Promise<UpToTenUnitsPage<Film>> => {
        return {
            units: mockFilms.slice(0, 10),
            hasNext: true,
            hasPrev: false
