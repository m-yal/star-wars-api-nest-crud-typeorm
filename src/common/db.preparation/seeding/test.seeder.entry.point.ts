...
export class TestSeeder implements MigrationInterface {

    name: string = 'TestSeeder1669806719729'; //do not change the value
    ...
    private throwErrorOrRecreateDir(): (err: Error) => void {
        return (err: Error) => {
            if (err) throw err;
            this.recreateDir(this.IMAGES_RELATIVE_FILE_PATH);
        }
    }

    private recreateDir(imagesPaht: string) {
        mkdir(imagesPaht, {}, (err) => {
            if (err) throw err;
            console.log("Directory for images recreated. Relative path to it: " + imagesPaht);
        });
    }
}